...
3.11 Shared Supabase Project (Infrastructure Consolidation)
Nucleus, DualPay, and valtaris-glue no longer run on three separate standalone Supabase projects. They now share one project, `valtaris-nucleus-2`, with each app's tables isolated in its own Postgres schema: nucleus's own tables stay in `public`, DualPay's live in a `dualpay` schema, and valtaris-glue's live in a `glue` schema. All three schemas share the same `auth.users` table, so a single Supabase Auth identity is now the common identity layer across the whole ecosystem instead of three disconnected user pools.

Each app's Supabase client is configured with `db.schema` pointed at its own schema; RLS policies and RPC/trigger functions were preserved per schema during the move (with an explicit `SET search_path` added anywhere a function relied on the old default `public`-only path); and PostgREST is configured to expose all three schemas via `pgrst.db_schemas = 'public, dualpay, glue'`. Both apps' Edge Functions were redeployed under this project, and both frontends were repointed and redeployed against it.
...
Security hardening (locked-down `SECURITY DEFINER` RPC surface, pinned `search_path`)	Implemented — 4 functions locked to service-role-only, 6 functions given an explicit `search_path`, verified against every real caller; see §3.12
