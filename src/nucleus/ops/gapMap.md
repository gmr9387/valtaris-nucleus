...
23. **Performance advisor sweep (this session).** Ran the full Supabase performance advisor (481 findings total across dualpay+public+glue; no ERROR-level findings anywhere). Fixed `idx_credentials_*`/`idx_connector_bindings_*` (8 unindexed FK columns, `20260925090000_add_missing_credential_indexes.sql`). Then closed **`auth_rls_initplan`** for this schema too (117 policies calling `auth.<function>()` directly instead of `(select auth.<function>())` in `USING`/`WITH CHECK`, re-evaluating per row instead of once per query): `20260925110000_public_schema_rls_perf_fix.sql`, the exact catalog-driven `DO` block already proven on dualpay's schema (`dualpay/20260924224814_...`, 134 policies) -- dry-run tested in a rolled-back transaction first, then applied; verified live (0 unwrapped remain) and the full test suite (88 tests) + typecheck still pass. `multiple_permissive_policies` (141 instances, e.g. `authorization_contract`'s `anon`/`INSERT` matching 3 stacked policies at once) is left open -- correct, just wasteful, needs real per-table policy-consolidation review rather than a mechanical rewrite.
