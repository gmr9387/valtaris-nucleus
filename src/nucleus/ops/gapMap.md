...
23. **Performance advisor sweep (this session, not a correctness bug): two real, mechanical, scoped-out perf debt items in this repo's own `public` schema.** Ran the full Supabase performance advisor (481 findings total across dualpay+public+glue; no ERROR-level findings anywhere). Fixed the two `public`-schema items with a clear, safe, immediate fix: `idx_credentials_*`/`idx_connector_bindings_*` (8 unindexed FK columns across `credentials`/`connector_bindings`, `20260925090000_add_missing_credential_indexes.sql`, both tables empty so free today). Left unfixed, deliberately, because they're large and not urgent: **`auth_rls_initplan`** (117 policies across this schema's tables -- `organizations`, `opportunity`, `execution`, `payment`, `credentials`, `connector_bindings`, `workflows`, etc. -- call `auth.<function>()` directly instead of `(select auth.<function>())` in their `USING`/`WITH CHECK`, which re-evaluates the auth call per row instead of once per query) and **`multiple_permissive_policies`** (141 instances -- several tables have 2-3 permissive policies stacked for the same role+action, e.g. `authorization_contract`'s `anon`/`INSERT` matches `guardian-contracts`, `org-isolation-authorization-insert`, and `service-role-full-authorization` all at once -- correct, just wasteful, since Postgres evaluates and ORs every one of them). Neither is a security or correctness bug -- every table involved is currently empty, so the cost is zero today -- but both are real, well-known Postgres RLS anti-patterns that get expensive once these tables hold production rows. The mechanical fix (wrap each `auth.uid()`/`auth.jwt()`/etc. call in `(select ...)`, no semantic change) is safe in principle but touches ~120 policy definitions across the schema; doing that as a blind bulk rewrite without reviewing each policy's actual logic was judged too risky to rush, so it's logged here as a real, known, prioritizable item rather than either ignored or done carelessly.
